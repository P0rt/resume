...
The homepage location now appears only in the footer as `Barcelona, 2026 ☯︎` (the build inserts the current year). Do not restore the location under the name; the full location remains available in structured profile data. The homepage and detailed work page intentionally ship no executable JavaScript; native media queries handle page and browser theme colors.
