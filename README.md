...
The homepage is a compact personal notebook: an introduction with a photo, current roles and a mention of the blog. It has no header controls or music block. Theme follows the browser or operating system, including live preference changes; old saved theme overrides are ignored. Detailed biography, projects, collaboration areas and career history live at `/work-together/`, linked from the introduction. The homepage and work page also have static Spanish, French, Portuguese, Japanese, Simplified Chinese and Russian versions under `/<locale>/`; Vercel Routing Middleware uses the browser's `Accept-Language` header only on the two English entry URLs. The full blog is a separate English page; individual article and project links do not appear on the homepage. Articles keep their own URLs. The resume PDF is retained in `src/assets/resume.pdf` for later use but is excluded from the public build.
...
Public profile facts live in `content/profile.json`; translated prose lives in `content/locales/*.json` and is joined to the same companies, dates, URLs and metrics during the build. The homepage, Person/ProfilePage schema and agent-readable profile therefore use the same factual source. Each translated home and work page has a self-canonical, reciprocal `hreflang` links, localized metadata and JSON-LD, and a corresponding sitemap entry. Each article has an explicit author, BlogPosting/BreadcrumbList data, canonical URL and Markdown alternate. The blog and article bodies are not translated.

Edit the homepage lead `intro`, career overview `homeStory`, current-work paragraph `homeCurrent` and `blogIntro` in `content/profile.json`. The company names in `homeCurrent` receive links from `currentRoles`; keep these names consistent. Full biography paragraphs, collaboration copy, current work, open projects and experience live in the same file. Experience entries can include `highlights` for additional paragraphs and `coverage` for a source link. The full profile remains available in `profile.json`, `index.md` and the MCP snapshot, including information moved to `/work-together/`. That page has its own canonical URL, AboutPage metadata and sitemap entry. The compact homepage layout lives in `src/styles/home.css`; the detailed work page uses `src/styles/profile.css`. Shared tokens and controls live in `src/styles/index.css`; archive and article typography live in `src/styles/blog.css`. `scripts/render-article.mjs` generates dual-theme syntax highlighting, heading permalinks and keyboard-scrollable tables at build time.

The homepage deliberately reads as a personal introduction, not three independent columns or a directory of Now/Writing rows. A large 240px portrait sits to the right of the name and introductory paragraph on desktop; on mobile the 192px portrait leads the page. The story below stays in one reading column. The September 22, 2026 approved copy covers software engineering, work at Yandex, TripleTen and Nebius, the information-systems bachelor’s degree and Stanford CS231n course, then current work as CTO at Aliwio and technical consultant at Symptomato. Keep the homepage company names free of Praktikum and Academy, and omit small-team wording, the pre-ChatGPT claim and the detailed Symptomato explanation. The user removed the detailed Yandex paragraph about being the 11th member, trainer architecture and daily students; keep those details on the work page and in the full agent-readable biography. The latest copy takes the narrative structure of the user's supplied reference (a decade of work, named projects, concrete outcomes and source links), never its author's co-founder claims, revenue, graduate counts, nonprofit work, awards or partners. Keep the graph-paper background, local Manrope body font, the original Iowan/Palatino/Georgia serif headings (explicitly requested by the user), and blue accent. The only homepage blog link belongs below the introduction, not the header. Do not restore the music block or theme switch, or add an article feed, full career history or extra sales sections to fill space. The music data remains in the profile source for future use. Motion is limited to a short entrance and hover feedback, disabled for reduced-motion preferences.
...
The homepage location now appears only in the footer as `Barcelona, 2026 ☯︎` (the build inserts the current year). Do not restore the location under the name; the full location remains available in structured profile data. Native media queries handle page and browser theme colors. The only executable script on the homepage and detailed work page is the optional, deferred PostHog analytics integration; all site content and navigation remain functional without it.

The homepage portrait uses a native `<picture>`: the supplied monochrome photograph (`src/assets/portrait-light.webp`) in light mode and the original blue photograph in dark mode. The WebP is a lossless encoding of the user-supplied PNG, with identical decoded pixels. Theme-specific preloads avoid downloading both portraits initially. No JavaScript or saved override controls the picture; it follows the browser's `prefers-color-scheme` setting, including changes while the page is open. The square frame, size and animation stay unchanged. Social previews and the primary structured-profile image continue to use the blue portrait.
...
Career corrections supplied by Sergei on September 3–4, 2026 take precedence over the older profile: he was Yandex Praktikum’s 11th team member, designed test architecture for a trainer serving 7,000–10,000 students daily and built ML project assessment before ChatGPT. His current roles are **Co-Founder and CTO** at [Aliwio](https://aliwio.com/) and **Tech Adviser** at [Symptomato](https://symptomato.com/). The September 22 homepage revision names [Nebius](https://nebius.com/) after TripleTen; its exact role and dates remain unspecified, so it is not yet a structured experience entry.
...
The read-only MCP endpoint is `https://sergei-parfenov.com/mcp`. `get_profile`, `search` and `fetch` accept `locale` values `en`, `es`, `fr`, `pt`, `ja`, `zh` and `ru`; an HTTP `Accept-Language` header is used only as the fallback. `MCP.md` documents connection options, tools, localized Markdown/JSON alternatives and security limits. The function is bundled from published content only. `npm test` validates both SEO output and MCP behavior before deployment.
...
PostHog Cloud EU project **276039** uses cookieless collection, explicitly selected by Sergei on September 16, 2026. The project token is a public ingestion token intentionally included in the browser bundle; it is not a personal API key and grants no dashboard access. No management credentials belong in this repository.

The build annotates page metadata and a small allowlist of real links, then bundles the pinned SDK separately from the entry module. Collection is limited to production hostnames, disabled by GPC/DNT and `?analytics=off`, and never gates navigation. Localhost and Vercel preview hosts do not send events. The approved schema, dashboard definitions and verification scenarios are in [ANALYTICS.md](ANALYTICS.md).

Only `$pageview`, `$pageleave`, `article_engaged`, `contact_clicked`, `profile_clicked`, `download_clicked` and `rss_clicked` are collected. There are no session recordings, automatic click capture, people profiles or cross-day identity. URLs and referrers are scrubbed, and campaign labels are restricted. Read the [privacy page](https://sergei-parfenov.com/privacy.html) for visitor-facing details.

For a supervised production check, `?analytics_test=1` labels the event `environment=test`; business dashboards must filter `environment=production`. This flag never enables collection on non-production hosts and is removed from reported URLs. Use it on every tested page; it is deliberately not persisted. Your own unmarked production visits can appear in reports; use browser GPC/DNT or the page-level opt-out when appropriate.

Cookieless session identifiers are assigned by the PostHog server. Verify the ingested `$session_id` across an article → home → work journey before relying on session-based funnels. Do not substitute user-based conversion for session conversion or call a mailto click a lead.
