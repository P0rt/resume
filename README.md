...
The homepage deliberately has one reading flow, not three independent columns: an introduction with a portrait, followed by short current-work and writing rows. Keep the graph-paper background, local Manrope font and blue accent. The only homepage blog link belongs in the Writing section, not the header. Do not restore the music block or theme switch, or add an article feed, full career history or extra sales sections to fill space. The music data remains in the profile source for future use. Motion is limited to a short entrance and hover feedback, disabled for reduced-motion preferences.
